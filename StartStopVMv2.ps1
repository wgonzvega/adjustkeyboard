...
# Peticiones
# 6/4/2021 no debe permitir busquedas a VM de produccion (LM) - en progreso
...
# produccion ya que encontrara la porcion prod o prd dentro del nombre del RG o VM
# 
...
function stopstartvm {
    if ($foundit -eq "1") {
        ...
        if ($selvm1 -match "prd" -or $foundrg -match "prd" -or $selvm1 -match "prod" -or $foundrg -match "prod" ) {
            if ($foundrg -ne "preprod" -or $foundrg -ne "pre-prod") {
